...
class Flower(pygame.sprite.Sprite):
    ...
    def growing(self):
        if (self.size > 0 and self.size <= 250) and \
                self.last_growing <= 0:
...
class Bug(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if (y < self.desty):
            self.dy += 1
        if (self.last_change > 100 and self.attack is False):
            self.attack = True
        ...
        if (self.attack and not self.done):
            ...
            if (self.suck_frames > 5 or self.change_times > 30):
                self.success()
        if self.done and \
                (self.rect.left > SCREENRECT.right or
                 self.rect.top > SCREENRECT.bottom):
            self.kill()
        if (not(self.done) and self.flower.is_alive() is False):
            self.success()
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.unfire()
        self.rect = self.image.get_rect(
            bottom=(SCREENRECT.height - MARGINBOTTOM))
        self.sprays_left = 0
...
def game():
    ...
    # Load Sounds
    Bug.sounds = [
        pygame.mixer.Sound(data.filepath('sounds', 'cough.wav')),
        pygame.mixer.Sound(data.filepath('sounds', 'laugh.wav'))]
    ...
    while 1:
        ...
        while game:
            ...
            while playing:
                ...
                if (attackers > 0 and attacker_last > attacker_last_time) and \
                        start_pause <= 0:
                ...
                if current_level >= 3 and \
                        bee_chance >= max_bee_chance and not \
                        bee_upset:
                    Bee((0, random.randrange(100, 200)))
                    bee_chance = 0
                for bug in (pygame.sprite.groupcollide(
                        bugs,
                        spray,
                        False,
                        False)):
                    bug.runaway(score)
                for bee in (pygame.sprite.groupcollide(
                        bees,
                        spray,
                        False,
                        False)):
                    bee.runaway()
                    bee_upset = True
                bee_flower_collide = pygame.sprite.groupcollide(
                    bees,
                    flowers,
                    False,
                    False)
